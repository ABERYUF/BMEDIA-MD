...
export default {
 name:"sh",aliases:[],category:"TOOLS",description:"Create a .sh file from text.",
 async execute(ctx){
  ...
  if(!name)name="bmedia"; if(!name.toLowerCase().endsWith(".sh"))name+=".sh";
  if(!body)return sock.sendMessage(from,{text:box("*SH*",["Reply to text: .sh filename","Or: .sh filename <content>","Default: bmedia.sh"])},{quoted:m});
  return sock.sendMessage(from,{document:Buffer.from(body),fileName:name,mimetype:"text/x-shellscript"},{quoted:m});
 }
};
