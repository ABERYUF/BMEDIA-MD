...
export default {
 name:"xml",aliases:[],category:"TOOLS",description:"Create a .xml file from text.",
 async execute(ctx){
  ...
  if(!name)name="bmedia"; if(!name.toLowerCase().endsWith(".xml"))name+=".xml";
  if(!body)return sock.sendMessage(from,{text:box("*XML*",["Reply to text: .xml filename","Or: .xml filename <content>","Default: bmedia.xml"])},{quoted:m});
  return sock.sendMessage(from,{document:Buffer.from(body),fileName:name,mimetype:"application/xml"},{quoted:m});
 }
};
