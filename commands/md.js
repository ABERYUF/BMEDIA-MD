...
export default {
 name:"md",aliases:["markdown"],category:"TOOLS",description:"Create a .md file from text.",
 async execute(ctx){
  ...
  if(!name)name="bmedia"; if(!name.toLowerCase().endsWith(".md"))name+=".md";
  if(!body)return sock.sendMessage(from,{text:box("*MD*",["Reply to text: .md filename","Or: .md filename <content>","Default: bmedia.md"])},{quoted:m});
  return sock.sendMessage(from,{document:Buffer.from(body),fileName:name,mimetype:"text/markdown"},{quoted:m});
 }
};
