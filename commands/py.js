...
export default {
 name:"py",aliases:[],category:"TOOLS",description:"Create a .py file from text.",
 async execute(ctx){
  ...
  if(!name)name="bmedia"; if(!name.toLowerCase().endsWith(".py"))name+=".py";
  if(!body)return sock.sendMessage(from,{text:box("*PY*",["Reply to text: .py filename","Or: .py filename <content>","Default: bmedia.py"])},{quoted:m});
  return sock.sendMessage(from,{document:Buffer.from(body),fileName:name,mimetype:"text/x-python"},{quoted:m});
 }
};
