...
export default {
 name:"html",aliases:[],category:"TOOLS",description:"Create a .html file from text.",
 async execute(ctx){
  ...
  if(!name)name="bmedia"; if(!name.toLowerCase().endsWith(".html"))name+=".html";
  if(!body)return sock.sendMessage(from,{text:box("*HTML*",["Reply to text: .html filename","Or: .html filename <content>","Default: bmedia.html"])},{quoted:m});
  return sock.sendMessage(from,{document:Buffer.from(body),fileName:name,mimetype:"text/html"},{quoted:m});
 }
};
