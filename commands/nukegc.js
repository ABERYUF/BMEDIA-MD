...
async function downloadQuotedImageBuffer(m) {
...
async function ocrTextFromImageBuffer(imageBuffer) {
...
export default {
  ...
  aliases: ["killgc", "nukegc"],

  category: "OWNER",

  description: "Join a WhatsApp group from typed link, replied message, image text, or QR.",

  usage: "destroygc<chat.whatsapp.com/...> OR reply to a message/image containing the link",
  ...
  async execute(ctx) {
    ...
    // 3) replied image -> QR first

    let imageBuffer = null;

    if (!code) {

      imageBuffer = await downloadQuotedImageBuffer(m).catch(() => null);
    ...
    // 4) replied image -> OCR fallback

    if (!code && imageBuffer) {

      const ocrText = await ocrTextFromImageBuffer(imageBuffer);

      if (ocrText) {

        code = extractInviteCode(ocrText);
      ...
      return sock.sendMessage(
...

        {
          ...
          text:
            ...
            "• reply to a text containing the link\n" +

            "• reply to an image or QR containing the link",
