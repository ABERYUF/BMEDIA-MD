...
export default {
 name:"csv",aliases:[],category:"TOOLS",description:"Create a .csv file from text.",
 async execute(ctx){
  ...
  if(!name)name="bmedia"; if(!name.toLowerCase().endsWith(".csv"))name+=".csv";
  if(!body)return sock.sendMessage(from,{text:box("*CSV*",["Reply to text: .csv filename","Or: .csv filename <content>","Default: bmedia.csv"])},{quoted:m});
  return sock.sendMessage(from,{document:Buffer.from(body),fileName:name,mimetype:"text/csv"},{quoted:m});
 }
};
