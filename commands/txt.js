...
export default {
 name:"txt",aliases:["text"],category:"TOOLS",description:"Create a .txt file from text.",
 async execute(ctx){
  ...
  if(!name)name="bmedia"; if(!name.toLowerCase().endsWith(".txt"))name+=".txt";
  if(!body)return sock.sendMessage(from,{text:box("*TXT*",["Reply to text: .txt filename","Or: .txt filename <content>","Default: bmedia.txt"])},{quoted:m});
  return sock.sendMessage(from,{document:Buffer.from(body),fileName:name,mimetype:"text/plain"},{quoted:m});
 }
};
