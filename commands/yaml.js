...
export default {
 name:"yaml",aliases:["yml"],category:"TOOLS",description:"Create a .yaml file from text.",
 async execute(ctx){
  ...
  if(!name)name="bmedia"; if(!name.toLowerCase().endsWith(".yaml"))name+=".yaml";
  if(!body)return sock.sendMessage(from,{text:box("*YAML*",["Reply to text: .yaml filename","Or: .yaml filename <content>","Default: bmedia.yaml"])},{quoted:m});
  return sock.sendMessage(from,{document:Buffer.from(body),fileName:name,mimetype:"application/yaml"},{quoted:m});
 }
};
